...
## 简介
Pika 手持夹爪用 lighthouse 基站定位,**基站原点逐 session 漂移**,没法直接对到松灵 robot base。本工具:每条录制都从**同一固定工装**起步,先把 pika 尖端轨迹 rebase 到首帧(消掉基站漂移),再经一次性标定的刚体变换 `ᴮʳT_P0` 搬进 robot base。

- **输入**:pika 同步 hdf5(`observations/pose_left|right`)或 pose 目录;**输出**:robot base 下尖端轨迹。
- **精度** ~7mm(LOO 交叉验证),d(法兰→尖端)=13.8cm,双臂。
- **用途**:把 pika episode 转到松灵系看位置 / 可达性;混训需再补朝向 Y(本版只搬位置)。

## 目录
- [核心公式](#核心公式)
- [硬性前提](#硬性前提)
- [模块](#模块)
- [采集协议](#采集协议) — 每臂一条、8 点铺开、各停 5s
- [用法](#用法) — 打包 → 提点 → 标定+LOO → 转换(单臂 / synced hdf5 双臂)
- [调参排坑](#调参排坑)
- [精度判读](#精度判读) — 看 LOO 不是拟合
- [已知边界](#已知边界)
